    ...
    public class ArcadeKart : MonoBehaviour
    {
        ...
        void ResetIfStuck()
        {
            if( IsStuck() && lastGroundCollided != null)
            {
                ...
                {
                    // Look for Road beneath kart
                    Ray groundCast = new Ray(transform.position, Vector3.down);

                    if (Physics.Raycast(groundCast, out RaycastHit hitInfo, 200f) )
                    {
                        if( hitInfo.collider.gameObject.CompareTag("Road") )
                        {
                            Vector3 pos = new Vector3(
                                hitInfo.point.x,
                                hitInfo.point.y + MinHeightThreshold,
                                hitInfo.point.z
                            );
        ...
        {
            var roadPieces = GameObject.FindGameObjectsWithTag("Road");

            GameObject closestFound = null;
            float shortestDist = float.PositiveInfinity;
            float distBetween = float.PositiveInfinity;

            foreach (var road in roadPieces )
            {
                distBetween = (road.transform.position - transform.position).magnitude;
